...
// Shared WebIDL infrastructure — ONE internal module backing every
// `src/web/*.js` consumer. Ports Deno's `ext/webidl/00_webidl.js`:
//   - `const { core, internals, primordials } = __bootstrap;`
//     → `const { primordials } = globalThis.__bootstrap;`
//   - `core.isProxy(V)` (the only `core.*` use in Deno's webidl) → the
//     Rust op `op_is_proxy(V) -> boolean` in `core::ops` (V8's
//     `Value::is_proxy` is genuinely irreducible native work, like the
//     other ops). Used by `createRecordConverter`'s fast path to reject
//     proxies that would break the brand/own-keys checks.
//   - `core.isArrayBuffer` / `core.isDataView` / `core.isTypedArray` /
//     `core.isSharedArrayBuffer` (Deno backs these with Rust ops too)
//     → pure-JS prototype checks via primordials. Limun has no
//     `SharedArrayBuffer`, so `isSharedArrayBuffer` is a constant `false`
//     (the `allowShared` opts paths collapse to the non-shared branch,
//     which is exactly the spec-correct behavior when SAB is absent).
//   - `internals.webidlBrand = brand;` → dropped. The only Deno reader
//     is a doc comment in `ext/crypto`; nothing in Limun reads
//     `internals.webidlBrand`. `brand` is exposed on the returned
//     `webidl` object (`webidl.brand`), which is the path every consumer
//     uses (`this[webidl.brand] = webidl.brand`).
//   - `return { … }` (Deno returns the webidl surface from the IIFE) →
//     `globalThis.__bootstrap.webidl = { … };` (Limun's established
//     cross-module pattern — see `00_primordials.js` /
//     `06_streams.js` / `09_blob.js`). Consumers do
//     `const webidl = globalThis.__bootstrap.webidl;`.
//
// The converter/brand/requiredArguments/dictionary/sequence/record/
// enum/nullable/promise/async-iterable/interface/iterator-mixin/
// setlike machinery is Deno's verbatim (spec-implementation code — the
// whole point of this module is to be the single shared copy).

((globalThis) => {
  const { primordials } = globalThis.__bootstrap;
  const { op_is_proxy } = globalThis.__limunOps;
  const {
  ...
  // --- `core.*` replacements (pure-JS prototype checks + the one op) -----
  //
  // Deno's `core.isArrayBuffer`/`isDataView`/`isTypedArray`/
  // `isSharedArrayBuffer` are backed by Rust ops that call V8's native
  // type checks. Here we use primordial prototype checks instead — same
  // observable result, no op boundary. Limun has no `SharedArrayBuffer`
  // at all, so `isSharedArrayBuffer` is a constant `false`; every
  // `allowShared` opt path in the converters collapses to the
  // non-shared branch (which is the spec-correct behavior when SAB
  // isn't part of the runtime).
  ...
  function isSharedArrayBuffer(_V) {
    return false;
  }
  // `core.isProxy` → `op_is_proxy` (V8 `Value::is_proxy`). The only
  // `core.*` use in Deno's webidl; needed by `createRecordConverter`'s
  // fast path so a Proxy with a `Symbol.iterator`/own-keys trap doesn't
  // sneak past the brand/own-keys checks.
  function isProxy(V) {
    return op_is_proxy(V);
  }
  ...
  // Expose the WebIDL surface on the bootstrap namespace (Limun's
  // cross-module pattern — see `00_primordials.js`). Consumers do
  // `const webidl = globalThis.__bootstrap.webidl;` and call
  // `webidl.converters.DOMString(...)`, `webidl.assertBranded(...)`,
  // `this[webidl.brand] = webidl.brand`, etc.
