...
//!
//! Module loading (`file:`/`https:` specifiers) routes through this today.
//! When the deno-like permission system lands (per MISSION.md), this is the
//! one place to add fs/net allowlist checks — `fetch()` and `Limun.fs.*`
//! are meant to route through these same two functions later, so a single
//! permission gate protects imports, `fetch`, and `fs` alike instead of
//! three separately-guarded call sites.
