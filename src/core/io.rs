...
//! separately-guarded call sites. Callers say *which mechanism* they are
//! (`Import` for module loading, `Fs` for a future `Limun.fs`, ...) so the
//! per-mechanism kill switches in `limun.json` apply; the target itself is
//! always checked against the unified `permissions.io` pattern list.

use crate::core::permissions::{self, Mechanism, Mode};
use std::path::Path;

/// Read a local file's contents as UTF-8 text.
pub fn read_file(path: &Path, mechanism: Mechanism) -> Result<String, String> {
    permissions::check_file(mechanism, path, Mode::Read)?;
    std::fs::read_to_string(path).map_err(|e| format!("cannot read {}: {e}", path.display()))
}

/// Fetch a remote URL's body as UTF-8 text (blocking).
pub fn fetch(url: &url::Url, mechanism: Mechanism) -> Result<String, String> {
    permissions::check(mechanism, url, Mode::Read)?;
